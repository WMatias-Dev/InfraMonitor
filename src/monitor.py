...
def obter_caminho_log():
    caminho_configurado = os.getenv("INFROMONITOR_LOG_FILE")
...
def resumir_metricas(registros):
    ...
    if disco:
        particoes = disco["valor"].get("particoes", [])
        percentuais = [particao.get("uso_percentual") for particao in particoes if particao.get("uso_percentual") is not None]
        if percentuais:
            disco_percentual = max(percentuais)
